...
public class Program
{
    ...
    private static Task<int> Main(string[] args)
    {
        ...
        {
            if (Utils.IsAppRunningAsAdministrator() && !existingInstanceIsAdmin &&
                TerminateOldInstance(existingInstance.ProcessId))
            {
    ...
    {
        AppInstance.FindOrRegisterForKey("ThreeFingerDragOnWindows-SingleInstance-" +
                                         (Utils.IsAppRunningAsAdministrator() ? "Admin" : "User"));
    ...
    {
        foreach (var appInstance in AppInstance.GetInstances())
        {
            if (appInstance.IsCurrent) continue;
            if (appInstance.Key.Equals("ThreeFingerDragOnWindows-SingleInstance-User")) return (appInstance, false);

            if (appInstance.Key.Equals("ThreeFingerDragOnWindows-SingleInstance-Admin")) return (appInstance, true);
        }
    ...
    {
        (Application.Current as App)?.DispatcherQueue.TryEnqueue(() =>
        {
    ...
    private static bool TerminateOldInstance(uint processId)
    {
        try
        {
            var oldInstance = Process.GetProcessById((int)processId);
            oldInstance.Kill();
        }
        catch (Exception ex)
        {
