...
public class FingerCounter
{
    ...
    private int
        _originalFingersCount; // Number of original fingers on the touchpad after the short delay. This is updated only when contacts list length is <= 1. 
    ...
    ///     shortDelayMovingFingersCount : number of fingers that are on the touchpad and that have led to a moving distance
    ///     higher than FINGERS_MOVE_THRESHOLD_SHORT
    ///     Used to determine what is the real number of fingers on the touchpad when contacts changed
    ///     longDelayMovingFingersCount : number of fingers that are on the touchpad and that have led to a moving distance
    ///     higher than FINGERS_MOVE_THRESHOLD
    ...
    ///     This is updated only when contacts list length is &lt;= 1 or when contacts have been released for more than
    ///     RELEASE_FINGERS_THRESHOLD_MS ms.
    ///     Used to determine if the user has originally started to scroll, drag, or desktop swipe. When moving with a single
    ///     finger, this variable is reset.
    /// </returns>
    public (int, int, int, int) CountMovingFingers(TouchpadContact[] newContacts, bool areContactsIdsCommons,
        float longestDist2D, bool hasFingersReleased)
    {
        if (!areContactsIdsCommons && (newContacts.Length <= 1 || hasFingersReleased)) _originalFingersCount = 0;
        ...
        if (longestDist2D >= 1)
        {
            // Do not take in account shorter distances
        ...
        if (_longDelayFingersMove > FINGERS_MOVE_THRESHOLD_LONG)
        {
            ...
            _longDelayFingersMove = 0;
            if (_originalFingersCount <= 1) _originalFingersCount = newContacts.Length;
        }
