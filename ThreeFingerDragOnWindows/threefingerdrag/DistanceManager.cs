...
public class DistanceManager
{
    ...
    public (int, Point, float) GetLongestDist2D(TouchpadContact[] oldContacts, TouchpadContact[] newContacts,
        bool hasFingersReleased)
    {
        ...
        var longestDistId = 0;
        var longestDistPoint = Point.Empty;
        ...
        foreach (var newContact in newContacts)
        {
            ...
            foreach (var oldContact in oldContacts)
            {
                ...
                var dist2D = newContact.GetDist2D(oldContact);
    ...
    public static Point ApplySpeedAndAcc(Point delta, int elapsed)
    {
        ...
        if (App.SettingsData.ThreeFingerDragCursorAcceleration != 0)
        {
            ...
            var a = App.SettingsData.ThreeFingerDragCursorAcceleration /
                    10f; // Acceleration is multiplied by 10 in settings.
            pointerVelocity = (float)(0.7 +
                                      0.8 * Sigmoid(2.6 * a *
                                          (mouseVelocity - 1 + (3 - Math.Log2(0.8 / 0.3 - 1)) / (2.6 * a)) - 3));
